...
import { CheckCircle, Clock, MapPin, IndianRupee, Calendar, Hash, Truck } from "lucide-react";
...
export default function DeliveryHistory() {
  ...
  // --- LOGIC CHANGE: Rider only earns the Delivery Fee, not the total bill ---
  const totalRiderEarnings = orders.reduce((sum, order) => sum + (order.deliveryFee || 0), 0);
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20 px-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* Header & Total Earnings Card */}
      <div className="mb-10 mt-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-orange-500 opacity-80">Rider Archives</h2>
                ...
                <div className="flex flex-col md:items-end">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <p className="text-[8px] font-black px-2 py-0.5 bg-gray-500/10 rounded uppercase tracking-widest text-gray-500">
                      COD: ₹{order.totalPrice?.toFixed(0)}
                    </p>
                    <p className="text-[9px] font-bold text-gray-500 uppercase">
                      {order.orderDate 
                        ? new Date(order.orderDate).toLocaleString('en-IN', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }) 
                        : "ARCHIVED"}
                    </p>
                  </div>
                </div>
