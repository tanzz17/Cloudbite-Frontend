          ...
          <div className={`overflow-hidden rounded-[2.5rem] border ${
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className={`uppercase text-[10px] tracking-[0.2em] font-black border-b ${
                ...
                <tbody className={`divide-y ${isDarkMode ? "divide-white/5" : "divide-gray-50"}`}>
                  {filteredOrders.map((order) => (
                    <tr key={order.id} className="group hover:bg-indigo-500/[0.03] transition-colors cursor-default">
                      <td className="py-6 px-8">
                        <div>
                          <div className="font-black text-sm uppercase flex items-center gap-2">
                            #{order.id} 
                            <span className="text-[10px] font-medium opacity-40 lowercase">by {order.customerName}</span>
                          </div>
                          <div className="text-[10px] opacity-40 font-bold flex items-center gap-1 uppercase mt-1">
                            <FiMapPin size={10} className="text-indigo-500" /> {order.deliveryAddress?.substring(0, 35)}...
                          </div>
                        </div>
                      </td>
            ...
            {filteredOrders.length === 0 && (
              <div className="py-20 text-center">
                <p className="opacity-40 font-black uppercase tracking-[0.3em] text-xs">Zero records match your criteria</p>
              </div>
            )}
          </div>
        )}
