      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <h1 className="text-5xl font-black tracking-tighter mb-3">
              Welcome back, <span className="text-indigo-500">{user?.name?.split(' ')[0] || "Admin"}</span> 👋
            </h1>
            <p className="text-sm font-bold uppercase tracking-[0.2em] opacity-50 flex items-center gap-2">
              <FiActivity className={`${loading ? 'animate-pulse' : ''} text-indigo-500`} /> 
              {loading ? "Syncing Platform Records..." : "Platform Overview & Control Center"}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {statCards.map((card, idx) => (
            ...
            >
              <div className={`w-14 h-14 rounded-2xl ${card.bg} ${card.color} flex items-center justify-center text-2xl mb-6 transition-transform group-hover:scale-110 group-hover:rotate-6`}>
                {card.icon}
              </div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-50 mb-1">{card.label}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="flex items-center justify-between py-4 border-b border-white/5 last:border-0">
                <div className="flex items-center gap-4">
                  ...
                  <p className="text-sm font-medium opacity-80">
                    {loading ? "Establishing handshake with backend..." : "All services operational. Database nodes synced."}
                  </p>
                </div>
          ...
          <div className={`p-10 rounded-[3rem] border flex flex-col justify-center items-center text-center ${
            ...
            <p className="text-sm opacity-70 mb-6 font-medium">Encountering issues with kitchen onboarding or payments?</p>
