...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className={`min-h-screen pt-20 flex flex-col md:flex-row transition-all duration-700 ${
      ...
      {/* Sidebar - Matching Kitchen/Customer Dashboard Style */}
      <aside className={`w-full md:w-80 p-8 md:fixed md:h-full transition-all duration-500 z-40 ${
          isDarkMode ? "bg-[#161b29] border-r border-white/5" : "bg-indigo-50/30 border-r border-indigo-100"
        }`}
      >
        {/* Brand/Back Link */}
        <button 
        ...
        <div className="mb-10">
            <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-indigo-500 opacity-80">Admin Console</h2>
            <h1 className="text-2xl font-black tracking-tighter mt-1">
              CLOUDBITE
            </h1>
            <div className="h-1.5 w-12 bg-indigo-500 mt-3 rounded-full"></div>
        </div>
      ...
      <main className="flex-1 md:ml-80 p-6 md:p-12 min-h-screen">
        <div className={`h-full min-h-[85vh] rounded-[3.5rem] p-8 md:p-12 ${
          isDarkMode ? "bg-[#161b29]/40 border border-white/5" : "bg-gray-50 border border-indigo-100/30"
        }`}>
            {children}
        </div>
      </main>
