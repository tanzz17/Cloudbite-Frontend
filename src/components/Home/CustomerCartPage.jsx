...
import { Trash2, Plus, Minus, ArrowLeft, ArrowRight, Loader2, ShoppingBag, Zap, CreditCard } from "lucide-react";
...
const updateQuantityApi = async (userId, foodId, quantity) => {
  const res = await axios.put(`${API_BASE_URL}/customers/cart/update/${userId}/${foodId}?quantity=${quantity}`);
  return res.data;
};

const removeItemApi = async (userId, cartItemId) => {
  const res = await axios.delete(`${API_BASE_URL}/customers/cart/remove/${userId}/${cartItemId}`);
  return res.data;
};
...
export default function CustomerCartPage() {
  ...
  if (loading) return (
    <div className={`min-h-screen flex flex-col justify-center items-center ${isDarkMode ? "bg-[#0b0f1a] text-white" : "bg-white text-gray-900"}`}>
      <Loader2 className="animate-spin w-10 h-10 text-orange-500 mb-4" />
      <p className="font-black uppercase tracking-widest text-xs italic">Syncing Cart...</p>
    </div>
  );
      ...
      <header className={`fixed top-0 left-0 right-0 z-40 border-b backdrop-blur-md transition-colors ${isDarkMode ? "bg-[#0b0f1a]/90 border-white/5" : "bg-white/90 border-gray-100"}`}>
        <div className="max-w-4xl mx-auto px-6 py-6 flex items-center justify-between mt-20">
          ...
          <div className="text-center">
            ...
            <p className="text-[9px] font-bold text-orange-500 uppercase tracking-[0.3em]">{cartItemCount} Verified Items</p>
          </div>
        ...
        {cartItemCount > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 space-y-4">
              {cart.items.map((item) => (
                <div key={item.id} className={`flex items-center p-5 rounded-[2rem] border transition-all ${
                    isDarkMode ? "bg-[#161b29] border-white/5 shadow-2xl" : "bg-white border-gray-100 shadow-xl shadow-gray-200/40"
                }`}>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">{formatPrice(item.priceAtAddition)} per unit</p>
                  ...
                  <div className="text-right flex flex-col items-end gap-3">
                    ...
                    <button onClick={() => handleRemove(item.id, item.foodName)} className="text-gray-500 hover:text-red-500 transition-colors">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
            ...
            <div className="lg:col-span-1">
                <div className={`p-8 rounded-[2.5rem] sticky top-32 border ${isDarkMode ? "bg-[#161b29] border-white/5" : "bg-white border-gray-100 shadow-2xl shadow-gray-200/50"}`}>
                    ...
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <span className="text-3xl font-black italic tracking-tighter text-orange-500">{formatPrice(cart.total)}</span>
                    </div>
          ...
          <div className="text-center py-20 flex flex-col items-center">
            ...
            <p className="text-xs font-bold text-gray-500 uppercase tracking-[0.3em] mb-10 max-w-xs leading-loose">No flavors detected in your current session.</p>
      ...
      {cartItemCount > 0 && (
          <div className="fixed bottom-10 left-0 right-0 z-30 flex justify-center px-6 md:hidden">
               <button onClick={() => navigate("/checkout")} className="w-full bg-orange-500 text-white p-5 rounded-2xl font-black italic flex items-center justify-between shadow-2xl">
                    <span className="flex items-center gap-2 tracking-tighter text-xl"><Zap size={20}/> {formatPrice(cart.total)}</span>
